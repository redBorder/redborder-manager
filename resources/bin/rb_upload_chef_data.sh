          ...
          if [ "x$key" != "x" ]; then
            ...
            for ((retry = 1; retry <= max_retries; retry++)); do
              ...
              if [ $RET -eq 0 ]; then 
                ...
                print_result $RET
                echo "Error uploading databag '$n2'. Retrying in $retry_delay seconds..."
                sleep $retry_delay
              fi

              # If all retries fail, exit
              if ((retry == max_retries)); then
                echo "Error: Failed to upload and verify databag '$n2' after $max_retries attempts."
              fi
            done
          else
            max_retries=3
          ...
          if [ "x$key" != "x" ]; then
            ...
            for ((retry = 1; retry <= max_retries; retry++)); do
              ...
              if [ $RET -eq 0 ]; then
                ...
                print_result $RET
                echo "Error uploading databag '$n1'. Retrying in $retry_delay seconds..."
                sleep $retry_delay
              fi

              # If all retries fail, exit
              if ((retry == max_retries)); then
                echo "Error: Failed to upload and verify databag '$n1' after $max_retries attempts."
              fi
            done
          else
            max_retries=3
        ...
        if [ "x$VAR" == "xy" ]; then
          ...
          for ((retry = 1; retry <= max_retries; retry++)); do
            ...
            if [ $? -eq 0 ]; then
              ...
              print_result $?
              echo "Error uploading $X '$n'. Retrying in $retry_delay seconds..."
              sleep $retry_delay
            fi
          
            # If all retries fail, exit
            if ((retry == max_retries)); then
              echo "Error: Failed to upload and verify $X '$n' after $max_retries attempts."
            fi
          done
        fi
      done
