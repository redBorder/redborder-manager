...
while [ $count -lt $retries ]; do
    sudo -u postgres pg_basebackup -h "$master" -D "$backup_dir" -U rep -R -v 2> /tmp/rb_notify_postgresql_pg_basebackup.log
    if [ $? -eq 0 ]; then
...
if [ $count -eq $retries ]; then
    ...
    cat /tmp/rb_notify_postgresql_pg_basebackup.log
    exit 1
fi
