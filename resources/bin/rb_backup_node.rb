...
nring                          = 168    # Max Number of backup files to store on S3
...
if opt['b']
  ...
  if (node_config["redBorder"]["manager"]["mode"] == "master") or (node_config["redBorder"]["manager"]["mode"] == "corezk") or (node_config["redBorder"]["manager"]["mode"] == "core")
    ...
    if opt['m']
      tarcmd = "nice -n 19 ionice -c2 -n7 #{tar} #{file_path} --exclude=/var/opt/chef-server/nginx/html --exclude=/root/.chef/syntax_check_cache --exclude=/var/chef/cookbooks --exclude=/var/chef/backup --exclude=/var/chef/data --exclude=/var/chef/backups --exclude=/var/opt/chef-server/rabbitmq/db --exclude=/var/opt/chef-server/bookshelf/data /root/.chef /etc/chef-server /etc/chef /var/chef /var/www/rb-rails/config /etc/redborder/mode /etc/hosts /etc/keepalived/keepalived.conf /var/pgdata/pg_hba.conf /var/opt/chef-server/nginx/ca /var/opt/chef-server/erchef/etc /var/opt/chef-server/chef-solr/etc /var/opt/chef-server/chef-expander/etc /var/opt/chef-server/rabbitmq/etc /etc/redborder/manager_index /tmp/#{hostname}-postgresql-dump-#{date}.gz /tmp/#{hostname}-backup-#{date}.txt"
    else
      tarcmd = "nice -n 19 ionice -c2 -n7 #{tar} #{file_path} --exclude=/var/opt/chef-server/nginx/html --exclude=/root/.chef/syntax_check_cache --exclude=/var/chef/cookbooks --exclude=/var/chef/backup --exclude=/var/chef/data --exclude=/var/chef/cache --exclude=/var/chef/backups --exclude=/var/opt/chef-server/rabbitmq/db --exclude=/var/opt/chef-server/bookshelf/data /root/.chef /etc/chef-server /etc/chef /var/chef /var/www/rb-rails/config /etc/mode /etc/hosts /etc/keepalived/keepalived.conf /var/pgdata/pg_hba.conf /var/opt/chef-server/nginx/ca /var/opt/chef-server/erchef/etc /var/opt/chef-server/chef-solr/etc /var/opt/chef-server/chef-expander/etc /var/opt/chef-server/rabbitmq/etc /etc/manager_index /tmp/#{hostname}-postgresql-dump-#{date}.gz /tmp/#{hostname}-backup-#{date}.txt"
    end
    ...
    printf "Actual node is not the master/core/corezk, exiting\n".colorize(:red)
    exit 0
  end
...
##################
  ...
  if File.exists?("/etc/redborder/externals.yml")
    externals             = YAML.load_file('/etc/redborder/externals.yml')
    domain_rbglobal       = externals['CDOMAIN']
    publicdomain_rbglobal = externals['PUBLICCDOMAIN']
  elsif File.exists?("/etc/redborder/manager.yml")
    manager_config        = YAML.load_file('/etc/redborder/manager.yml')
  ...
  if opt['n']
    verified = true
    check_oper("rm -f /etc/redborder/blocked/*; touch /etc/redborder/s3user.txt; rm -f /etc/redborder/cluster.lock", verbose, type, "Deleting locking files ... ")
  else
    node_config = Chef::Node.load(hostname)
    if ( !node_config.nil? and !node_config["redBorder"].nil? and !node_config["redBorder"]["manager"].nil? and ( (node_config["redBorder"]["manager"]["mode"] == "master") or (node_config["redBorder"]["manager"]["mode"] == "corezk") or (node_config["redBorder"]["manager"]["mode"] == "core") ) )
      verified = true
    end
  end
  ...
  if verified
    ...
    failed = true if (opt['m']) and (`tar -tf #{path} | grep "var/chef/cache/cookbooks"` == "")
    ...
    # copy default license if the server has no license
    `cp /var/chef/cookbooks/redBorder-manager/files/default/rB.lic /var/www/rb-rails/` if !File.exist?"/var/www/rb-rails/rB.lic" and File.exist?"/var/chef/cookbooks/redBorder-manager/files/default/rB.lic"
    ...
    if opt['n']
      ...
      if !domain_restore.nil? and domain_restore != domain_rbglobal
        ...
        [ domain_rbglobal, "chefwebui.#{domain_rbglobal}", "data.#{domain_rbglobal}", "erchef.#{domain_rbglobal}", "repo.#{domain_rbglobal}", "s3.#{domain_rbglobal}", "webui.#{domain_rbglobal}" ].each do |cert|
          ...
          check_oper("/usr/lib/redborder/bin/rb_create_cert.sh -n #{cert}", verbose, type, "Creating #{cert} cert ...")
          check_oper("/usr/lib/redborder/bin/rb_upload_certs.sh #{cert}"  , verbose, type, "Uploading #{cert} cert ...")
        end
      end

      check_oper("mkdir -p /root/.chef/trusted_certs/; rsync /var/opt/chef-server/nginx/ca/erchef.#{domain_rbglobal}.crt /var/opt/chef-server/nginx/ca/#{domain_rbglobal}.crt /root/.chef/trusted_certs/; mkdir -p /home/redBorder/.chef/trusted_certs/; rsync /var/opt/chef-server/nginx/ca/erchef.#{domain_rbglobal}.crt /var/opt/chef-server/nginx/ca/#{domain_rbglobal}.crt /home/redBorder/.chef/trusted_certs/; chown -R redBorder:redBorder /home/redBorder/.chef", verbose, type, "Copying certs to trusted certs")

      # Change domain an public domain for database
      db_list = ["db_opscode_chef","db_druid","db_oozie","db_redborder"]
      db_list.each { |db|
      ...
      FileUtils.rm_f("/etc/redborder/extrahosts") if File.exists?"/etc/redborder/extrahosts"
      ...
      if !new_domain.nil? and new_domain["name"] != domain_rbglobal
        `echo "127.0.0.1 erchef.#{new_domain["name"]} postgresql.#{new_domain["name"]}" >> /etc/redborder/extrahosts` if opt['n']
    ...
    `echo "127.0.0.1 erchef.#{domain_rbglobal} postgresql.#{domain_rbglobal}" >> /etc/redborder/extrahosts` if opt['n']
    ...
    check_oper("rb_service stop rabbitmq; rm -rf /var/opt/chef-server/rabbitmq/db/*; rb_service start rabbitmq; sleep 5; /usr/lib/redborder/bin/rb_create_rabbitusers.sh; rm -f /etc/rabbitmq/users_created", verbose, type, "Removing rabbitmq data ... ")
    ...
    message = "Deleting temporal files ... "
    if opt['f']
      check_oper("nice -n 19 ionice -c2 -n7 rm -f /tmp/*-postgresql-dump-*; nice -n 19 ionice -c2 -n7 rm -f #{file_config}; rm -f /etc/redborder/extrahosts", verbose, type, message)
    else
      check_oper("nice -n 19 ionice -c2 -n7 rm -f /tmp/*-postgresql-dump-*; nice -n 19 ionice -c2 -n7 rm -f #{file_config}; nice -n 19 ionice -c2 -n7 rm -f /tmp/#{file_to_restore}; rm -f /etc/redborder/extrahosts", verbose, type, message)
    end
